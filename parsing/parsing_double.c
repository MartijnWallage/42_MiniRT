...
{
	int		pos_point;
	char	int_part[MAX_DIGETS_DOUBLE_INT_PART + 1];
	char	frac_part[MAX_DIGETS_DOUBLE_FRAC_PART + 1];
	...
	if (pos_point < 0)
		return (0);
	if (pos_point > MAX_DIGETS_DOUBLE_INT_PART || (int)ft_strlen(str) \
		- pos_point > MAX_DIGETS_DOUBLE_FRAC_PART + 1)
			return (0);
